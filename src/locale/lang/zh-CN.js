export default {
	home: '首页',
	login: '登录',
	components: '组件',
	count_to_page: '数字渐变',
	tables_page: '多功能表格',
	split_pane_page: '分割窗口',
	markdown_page: 'Markdown编辑器',
	editor_page: '富文本编辑器',
	icons_page: '自定义图标',
	img_cropper_page: '图片编辑器',
	update: '上传数据',
	join_page: 'QQ群',
	doc: '文档',
	update_table_page: '上传CSV文件',
	update_paste_page: '粘贴表格数据',
	multilevel: '多级菜单',
	directive_page: '指令',
	level_1: 'Level-1',
	level_2: 'Level-2',
	level_2_1: 'Level-2-1',
	level_2_3: 'Level-2-3',
	level_2_2: 'Level-2-2',
	level_2_2_1: 'Level-2-2-1',
	level_2_2_2: 'Level-2-2-2',
	excel: 'Excel',
	'upload-excel': '上传excel',
	'export-excel': '导出excel',
	tools_methods_page: '工具函数',
	drag_list_page: '拖拽列表',
	i18n_page: '多语言',
	modalTitle: '模态框题目',
	content: '这是模态框内容',
	buttonText: '显示模态框',
	'i18n-tip': '注：仅此页做了多语言，其他页面没有在多语言包中添加语言内容',
	error_store_page: '错误收集',
	error_logger_page: '错误日志',
	query: '带参路由',
	params: '动态路由',
	cropper_page: '图片裁剪',
	message_page: '消息中心',
	tree_table_page: '树状表格',
	org_tree_page: '组织结构树',
	drag_drawer_page: '可拖动抽屉',
	tree_select_page: '树状下拉选择器',
	v_rtu_management: '货机管理',
	my_rtu: '我的机器',
	c_my_rtu: '我的机器',
	m_my_rtu: '我的机器',
	add_rtu: '添加机器',
	copy_rtu: '批量复制机器',
	replenishment: '补货',
	replenishment_record: '补货记录',
	shipping_detail: '出货明细',
	goods_management: '商品管理',
	add_goods: '添加商品',
	goods_list: '商品列表',
	add_goods_type: '添加分类',
	goods_type_list: '分类列表',
	order_management: '订单管理',
	m_order_management: '订单管理',
	correct_statistics: '纠正统计',
	m_order_earnings: '收益',
	m_sales_subsidiary: '销售明细',
	order_list: '订单列表',
	clean_order: '清理订单',
	statistical_analysis: '统计分析',
	earnings: '收益',
	sales_subsidiary: '销售明细',
	pay_management: '支付管理',
	pay_list: '支付列表',
	add_pay: '添加支付',
	user_management: '用户管理',
	add_user: '添加用户',
	c_add_user: '添加用户',
	user_list: '用户列表',
	c_user_list: '用户列表',
	company_management: '公司管理',
	add_company: '添加公司',
	company_list: '公司列表',
	m_earnings: '近日收益',
	m_order_dealing: '订单处理',
	m_replenishment: '补货',
	me: '用户信息',
	cooperation_management: '合作管理',
	cooperation_list: '合作列表',
	c_cooperation_list: '合作列表',
	add_cooperation: '添加合作',
	c_add_cooperation: '添加合作',
	data_analysis: '数据分析',
	curves_in: '曲线走势',
	hot_goods: '热销商品',
	active_users: '活跃用户',
	m_rtu_test: '测试机器',
	shop_type: '商店类型',
	add_shop_type: '添加类型',
	shop_type_list: '类型列表',
	ad_management: '广告管理',
	add_ad: '添加广告',
	ad_list: '广告列表',
	Network:'网络',
	Error:'错误'

}
