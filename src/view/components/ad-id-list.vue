<template>
	<div>
		<div style="overflow: hidden;padding-bottom: 8px;">
			<Input search enter-button placeholder="请输入广告名称关键字" @on-search="findAdList" style="width:50%;float: left;" />
		</div>
		<Table :loading="tableLoading" size="small" border :columns="adColumns" :data="adList">
			...
			<template slot-scope="{ row, index }" slot="action">
				<Button type="primary" size="small" @click="chooseAdIdInfo(row)">选择</Button>
			</template>
		</Table>
...
export default {
  ...
  methods: {
    ...
    getAdList () {
      ...
      getAdvertisementList(this.adName, this.rtuTypeTag, this.maxId, this.pageSize).then(res => {
        const data = res.data
